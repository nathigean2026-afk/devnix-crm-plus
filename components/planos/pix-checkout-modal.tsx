...
export function PixCheckoutModal({ open, onClose, planId, planName, planPrice }: PixCheckoutModalProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { if (!v) onClose() }}>
      <DialogContent
        className="bg-card border-border p-0 max-w-sm w-full overflow-hidden"
          ...
          {(state === "ready" || state === "polling") && qrCodeBlobUrl && (
            <div className="flex flex-col items-center gap-4">
              ...
              <div className="relative">
                ...
                {state === "polling" && (
                  <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 bg-card border border-border rounded-full px-2.5 py-0.5 flex items-center gap-1.5">
                    <div className="size-1.5 rounded-full bg-green-400 animate-pulse" />
                    <span className="text-xs text-muted-foreground whitespace-nowrap">Verificando...</span>
                  </div>
                )}
              </div>
